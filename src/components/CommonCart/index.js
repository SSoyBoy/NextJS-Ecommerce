...
export default function CommonCart({
  ...
  const quantities = (product, amount) => {
    const idx = cartItems.indexOf(product);
    const arr = [...cartItems];
    arr[idx].quantity += amount;
    if (arr[idx].quantity == 0) arr[idx].quantity = 1;
    setCartItems([...arr]);
    updateCart({
      userID: product.userID,
      productID: product.productID._id,
      quantity: arr[idx].quantity,
    });
  };
              ...
              <div className="mt-6 border-t border-b py-2">
                ...
                <div className="mt-5 text-center">
                  <button
                    ...
                    className="disabled:opacity-50 group inline-flex w-full items-center justify-center bg-black hover:bg-gray-600 px-6 py-4 text-lg text-white font-medium uppercase tracking-wide"
                  >
                    Checkout
                  </button>
                </div>
