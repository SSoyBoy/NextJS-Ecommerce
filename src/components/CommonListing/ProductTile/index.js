...
export default function ProductTile({ item }) {
  ...
  return (
    <div onClick={() => router.push(`/product/${item._id}`)}>
      <div className="overflow-hideen aspect-w-1 aspect-h-1 h-52 overflow-hidden">
        <img
          src={item?.imageUrl}
          alt="Product image"
          className="h-full w-full object-cover object-center transition-all duration-300 hover:scale-110"
        />
      </div>
      {item.onSale === "yes" ? (
      ...
      <div className="my-4 mx-auto flex w-10/12 flex-col items-start justify-between">
        <div className="mb-2 flex truncate w-full">
          <p
        ...
        <h3 className="md-2 w-full text-gray-400 text-sm truncate">
          {item.name}
        </h3>
      </div>
    </div>
